class Sim:
    ...
    def use_bathroom(self):
        if self.alive:
            self.adjust_need('bladder', 10 - self.bladder)
    ...
    def sleep(self):
        if self.alive:
            self.adjust_need('energy', 10 - self.energy)
    ...
    def take_shower(self):
        if self.alive:
            self.adjust_need('hygiene', 10 - self.hygiene)
    ...
    def work(self, job, hours):
        if self.alive:
            ...
            print(
                f"{self.name} worked as a {job.name} for {hours} hours and earned ${salary}. Wallet: ${self.money}")
            self.show_needs()
